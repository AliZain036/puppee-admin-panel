...
export default class GalleryForm extends React.Component {
  ...
  postBrand(event) {
    ...
    if (match.params.brandId) {
      ...
      axios
        .post(`${API_END_POINT}/api/brands/update`, brand, {
          headers: { "auth-token": token },
        })
        .then((response) => {
      ...
      axios
        .post(`${API_END_POINT}/api/add-color`, brand, {
          headers: { "auth-token": token },
        })
        .then((response) => {
          if (response.status === 200) {
            Swal.fire({
              ...
              text:
                "Color has been added successfully.Redirecting in 2 seconds.",
            }).then(() => {
              console.log("YESSSS");
              window.history.back();
            });

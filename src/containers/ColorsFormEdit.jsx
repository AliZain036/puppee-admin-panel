...
export default class GalleryForm extends React.Component {
  constructor(props) {
    ...
    this.state = {
      loading: false,
      brand: {
        name: this.props.location.dataToPass.brand.name,
        description: this.props.location.dataToPass.brand.description,
      },
    };
  ...
  componentDidMount() {
    ...
    if (match.params.brandId) {
      axios
        .get(`${API_END_POINT}/api/brands/one`, {
          params: { brandId: match.params.brandId },
  ...
  postBrand(event) {
    ...
    if (match.params.brandId) {
      var data = qs.stringify({
        brand_id: this.props.location.dataToPass.brand.id,
        name: brand.name,
        description: brand.description,
      });
      brand.brandId = match.params.brandId;
      axios
        .post(`${API_END_POINT}/api/update-brand`, data, {
          headers: { "auth-token": token },
        })
        .then((response) => {
      ...
      var data = qs.stringify({
        color_id: this.props.location.dataToPass.brand.id,
        name: brand.name,
      });
      axios
        .post(`${API_END_POINT}/api/update-color`, data, {
          headers: { "auth-token": token },
        })
        .then((response) => {
          if (response.status === 200) {
